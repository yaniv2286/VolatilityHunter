...
strategy_v8_1_1.py
==================
VolatilityHunter v8.1.1 — Trailing Stop ONLY
Single improvement: Add trailing stop for standard positions.
All other v8.1 logic unchanged.
...
v8.1 baseline preserved:
  ...
  - Single-day volume surge (1.5× SMA30) - NOT CHANGED
"""
...
# ── V8.1.1 Parameters ────────────────────────────────────────────────────────────
# Same as v8.1 EXCEPT for trailing stop
HARD_STOP_PCT       = 0.08
...
def backtest_ticker_v8_1_1(ticker: str, df: pd.DataFrame,
                           spy_sma200: Optional[pd.Series] = None,
                           sector: str = 'Unknown') -> List[dict]:
    """
    Single-ticker backtest for v8.1.1.
    ONLY CHANGE: Add trailing stop for standard positions (2× ATR from highest).
    ...
    try:
        ...
        for i in range(1, n):
            if not in_position:
                ...
                elif not np.isnan(atr[i]) and atr[i] > 0:
        ...
        import traceback
        import logging
        logging.getLogger('strategy_v8_1_1').debug(
            f"backtest_ticker_v8_1_1 {ticker}: {e}\n{traceback.format_exc()}")
        return []
