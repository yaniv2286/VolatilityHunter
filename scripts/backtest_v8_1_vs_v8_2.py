"""
backtest_v8_1_vs_v8_2.py
========================
Compare v8.1 (baseline) vs v8.2 (audit improvements).
v8.2 improvements:
  1. Trailing stop for standard positions (2× ATR from highest)
  2. Multi-day volume confirmation (2 consecutive days)
  3. Trading days for time stop (not calendar days)
  4. Normalized scoring (future enhancement)
  5. Real GICS sectors (future enhancement)
...
from src.strategy_v8_2 import backtest_ticker_v8_2
from src.strategy_v7_2 import add_indicators_v7_2

logging.basicConfig(level=logging.WARNING, format='%(asctime)s %(levelname)s %(message)s')
logger = logging.getLogger('backtest_v81_v82')
...
def build_equity_curve_v8_1(all_trades: List[dict],
    ...
    """v8.1 equity curve with regime-aware max positions and sector cap."""
    if not all_trades:
...
def print_comparison(m81: dict, m82: dict):
    def delta(v82, v81, higher_is_better=True):
        d = v82 - v81
        sign = '+' if d >= 0 else ''
        if higher_is_better:
    ...
    print()
    print("=" * 72)
    print("STRATEGY COMPARISON: v8.1 (baseline)  vs  v8.2 (audit improvements)")
    print("v8.2 adds: trailing stop + multi-day volume + trading-day time stop")
    print("=" * 72)
    print(f"{'Metric':<28} {'v8.1':>10} {'v8.2':>10} {'Delta':>14}")
    print("-" * 72)
    rows = [
        ('26yr CAGR %',     'cagr',          True),
    ...
    for label, key, hib in rows:
        ...
        v82v = m82.get(key, 0)
        if key == 'final_equity':
            print(f"  {label:<26} {v81v/1000:>10.1f} {v82v/1000:>10.1f} {delta(v82v/1000, v81v/1000, hib):>14}")
        elif key == 'total_trades':
            print(f"  {label:<26} {int(v81v):>10} {int(v82v):>10}")
        else:
            print(f"  {label:<26} {v81v:>10.2f} {v82v:>10.2f} {delta(v82v, v81v, hib):>14}")
    print("=" * 72)
    print()
    dd_d   = m82.get('max_drawdown', 0) - m81.get('max_drawdown', 0)
    cagr_d = m82.get('cagr', 0) - m81.get('cagr', 0)
    if dd_d < 0 and cagr_d >= 0:
        verdict = "v8.2 WINS  (lower DD, CAGR improved or preserved)"
    elif dd_d < 0 and cagr_d >= -1.0:
        verdict = "v8.2 MIXED (lower DD but CAGR cost < 1%)"
    elif cagr_d > 0 and dd_d <= 0:
        verdict = "v8.2 WINS  (higher CAGR, DD improved or same)"
    else:
...
def main():
    ...
    print("=" * 72)
    print("VOLATILITYHUNTER BACKTEST: v8.1 vs v8.2 (AUDIT IMPROVEMENTS)")
    print(f"Universe: {len(parquet_files)} tickers | Capital: ${INITIAL_CAPITAL:,.0f}")
    print("v8.2: trailing stop + multi-day volume + trading-day time stop")
    print("=" * 72)
    ...
    for i, fpath in enumerate(parquet_files):
        ...
        if (i + 1) % 300 == 0:
            print(f"  {i+1}/{len(parquet_files)} | v8.1:{len(v81_trades)} v8.2:{len(v82_trades)} trades")
        ...
        v81_trades.extend(backtest_ticker_v8_1(ticker, df,
                                                ...
                                                sector=sector))
        v82_trades.extend(backtest_ticker_v8_2(ticker, df,
                                                spy_sma200=spy_regime,
                                                sector=sector))
        loaded += 1

    print(f"  Done: {loaded} tickers | {skipped} skipped")
    print(f"  v8.1 trades: {len(v81_trades)} | v8.2 trades: {len(v82_trades)}")

    if not v81_trades or not v82_trades:
        print("ERROR: No trades generated.")
        return 1

    print("\nBuilding equity curves...")
    ec81 = build_equity_curve_v8_1(v81_trades, spy_regime)
    ec82 = build_equity_curve_v8_1(v82_trades, spy_regime)  # Use same equity curve logic

    m81 = calc_metrics(ec81, v81_trades)
    m82 = calc_metrics(ec82, v82_trades)

    print_comparison(m81, m82)

    # Breakdown by exit reason for v8.2
    trailing_stops = [t for t in v82_trades if 'Trailing stop' in t.get('reason', '')]
    if trailing_stops:
        ts_avg = float(np.mean([t['pnl_pct'] for t in trailing_stops]))
        ts_winners = sum(1 for t in trailing_stops if t['pnl_pct'] > 0)
        print(f"v8.2 Trailing stop trades: {len(trailing_stops)} | avg P&L: {ts_avg:.2f}% | winners: {ts_winners}")

    # Volume confirmation impact
    print(f"\nv8.2 Trade count delta: {len(v82_trades) - len(v81_trades)} (multi-day volume filter)")
    ...
    out = {
        ...
        'v8_1':            m81,
        'v8_2':            m82,
        'delta_cagr':      m82.get('cagr', 0)         - m81.get('cagr', 0),
        'delta_drawdown':  m82.get('max_drawdown', 0)  - m81.get('max_drawdown', 0),
        'delta_sharpe':    m82.get('sharpe', 0)        - m81.get('sharpe', 0),
        'trailing_stop_count': len(trailing_stops),
        'trade_count_delta': len(v82_trades) - len(v81_trades),
    }
    ts = pd.Timestamp.now().strftime('%Y%m%d_%H%M')
    out_path = OUTPUT_DIR / f"backtest_v8_1_vs_v8_2_{ts}.json"
