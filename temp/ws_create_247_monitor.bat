...
:: Delete the old broken task first
schtasks /delete /tn "Auto_IBGateway_Manager" /f >nul 2>&1

:: Create the new 24/7 task
schtasks /create /tn "IBGateway_Monitor_247" /tr "cmd /c 'D:\GitHub\VolatilityHunter\scripts\DAILY_ROUTINE\run_auto_tws_manager.bat'" /sc minute /mo 5 /ru SYSTEM /f /rl HIGHEST
